...
int main() {
	...
	if(G.hand[p][pre.handCount[p]-1]==-1 
		|| G.hand[p][pre.handCount[p]]==-1
		|| G.hand[p][pre.handCount[p]+1]==-1)
	...
	if(G.playedCards[pre.playedCardCount] != smithy)
	{
		printf("Error: played card not added to played pile\n");
	}
	
	if( (pre.deckCount[p]+pre.discardCount[p]-3) != (G.deckCount[p]+G.discardCount[p]) )
