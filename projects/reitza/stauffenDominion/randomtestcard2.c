...
int checkSmithy(int card, struct gameState *post,int p, int handPos){
  struct gameState pre;
  memcpy (&pre, post, sizeof(struct gameState));
  int choice1=1, choice2=1, choice3=1;
  int r,i, deckTreasure, discardTreasure, ddCount;
  ...
  ddCount= pre.deckCount[p]+pre.discardCount[p];
  //check found Count or Card errors
  if(pre.deckCount[p]+pre.discardCount[p]>=3){//Test for two treasures picked up 
    if(post->handCount[p]!=pre.handCount[p]+2)
      errorHandCount=1;
    if(post->deckCount[p]+post->discardCount[p]!=pre.deckCount[p]+pre.discardCount[p]-3)
      errorDeckDisCount=1;
  }else{
    if(post->handCount[p]!=pre.handCount[p]+ddCount-1)
      errorHandCount=1;
    if(post->deckCount[p]+post->discardCount[p]!=0)
      errorDeckDisCount=1;
  }
